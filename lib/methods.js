...
Meteor.methods({
  addSong: function (song, roomId) {
    ...
    Songs.insert(_.extend(getSong(song), {addedBy: this.userId, addedFrom: roomId, addedAt: new Date()}));
  },
  ...
  favorite: function (song) {
    if (!this.userId) throw new Meteor.Error(401, 'User not logged in');
    Favorites.insert(_.extend(getSong(song), {favoritedBy: this.userId}));
  },
